...
$DIR/server.sh "$example" "$subcommand" --id n0 --storage "${tmp}/n0" --peers $peers --raftGroup $group&
$DIR/server.sh "$example" "$subcommand" --id n1 --storage "${tmp}/n1" --peers $peers --raftGroup $group&
$DIR/server.sh "$example" "$subcommand" --id n2 --storage "${tmp}/n2" --peers $peers --raftGroup $group&

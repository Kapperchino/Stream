...
public class FileStoreSerializer extends StdSerializer<FileStore> {
    ...
    public void serialize(FileStore value, JsonGenerator gen, SerializerProvider provider) throws IOException {
        ...
        gen.writeNumberField("writeThreadNum", ConfUtils.getInt(properties::getInt, FileStoreCommon.STATEMACHINE_WRITE_THREAD_NUM,
                1, log::info));
        gen.writeNumberField("readThreadNum", ConfUtils.getInt(properties::getInt, FileStoreCommon.STATEMACHINE_READ_THREAD_NUM,
                1, log::info));
        gen.writeNumberField("commitThreadNum", ConfUtils.getInt(properties::getInt, FileStoreCommon.STATEMACHINE_COMMIT_THREAD_NUM,
                1, log::info));
        gen.writeNumberField("deleteThreadNum", ConfUtils.getInt(properties::getInt, FileStoreCommon.STATEMACHINE_DELETE_THREAD_NUM,
                1, log::info));
